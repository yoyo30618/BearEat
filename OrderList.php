					...
					<div class="" data-wow-duration="1s" data-wow-delay="0.6s" data-wow-offset="0">
						<div class="contact">
							<?php
								...
								$sql_Food="SELECT * FROM `ordertable` ORDER BY `ordertable`.`Status` DESC, `ordertable`.`GetDateTime` ASC";
								$res_Food=mysqli_query($db_link,$sql_Food)or die("sql_Lang查詢失敗");
							?>
							<form action="OrderChange.php" method="post" id="OrderForm">
								<table class="table table-Info table-hover table-bordered" style="text-align:center;">
									...
									<?php
										while($row_Food=mysqli_fetch_array($res_Food)){
											if($row_Food['Status']=="未取餐")
												echo "<tr style='color:red'>";
											else
												echo "<tr>";
												echo "<td>".$row_Food['_ID']."</td>";
												echo "<td>".$row_Food['OrderDateTime']."</td>";
												echo "<td>".$row_Food['GetDateTime']."</td>";
												echo "<td>".$row_Food['Name']."</td>";
												echo "<td>".$row_Food['Phone']."</td>";
												echo "<td>".$row_Food['OrderList']."</td>";
												echo "<td>".$row_Food['Price']."</td>";
												echo "<td>".$row_Food['Status']."</td>";
												if($row_Food['Status']=="未取餐"){
													echo "<td>";
														echo "<button type='submit' form='OrderForm' class='btn btn-success' name='FinishStatus".$row_Food['_ID']."'>到場取餐</button>  ";
														echo "<button type='submit' form='OrderForm' class='btn btn-danger' name='CancelStatus".$row_Food['_ID']."'>取消此單</button>";
													echo "</td>";
												}
												else if($row_Food['Status']=="已取餐")
													echo "<td><button type='submit' form='OrderForm' class='btn btn-primary' name='FixStatus".$row_Food['_ID']."'>修正未取</button></td>";
												else 
													echo "<td></td>";
											echo "</tr>";
										}	
